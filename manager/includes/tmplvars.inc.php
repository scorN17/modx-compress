	    ...
	    if ($cimode === false) {
			...
			//v005
				...
				if( $field_value_arr )
				{
					foreach( $field_value_arr AS $key => $row )
					{
						$rowa= explode( "::", $row );
						$field_html .= '<div>
							<input type="text" name="tv'. $field_id .'prnm[]"  value="'. $rowa[ 0 ] .'" '. $field_style .' /> &nbsp;
							<input type="text" name="tv'. $field_id .'prval[]"  value="'. $rowa[ 1 ] .'" '. $field_style .' /> &nbsp;
							<input type="button" value="-" onclick="scornRemoveParams'. $field_id .'( this )" />
						</div>';
					}
				}
			...
			//v005
				...
				if( $field_value_arr )
				{
					...
					{
						$rowa= explode( "::", $row );
						$field_html .= '<div><input type="text" id="tv'. $field_id .'_'. $key .'" name="tv'. $field_id .'climg[]"  value="'. $rowa[0] .'" '. $field_style .' onchange="documentDirty=true;" />&nbsp;<input type="button" value="'. $_lang['insert'] .'" onclick="BrowseServer( \'tv'. $field_id .'_'. $key .'\' )" />&nbsp;&nbsp;<input type="button" value="-" onclick="scornImageRemove'. $field_id .'( this )" /></div><div><img style="max-width:300px;max-height:100px;" src="http://'. $_SERVER[ 'SERVER_NAME' ] .'/'. $rowa[0] .'" /></div>';
					}
				}
