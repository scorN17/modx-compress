...
while ($row = $modx->db->getRow($rs)) {
	...
	switch ($row['type']) {
		...
		//v005
			...
			{
				foreach( $tmplvar_arr_prnm AS $key => $val )
				{
					$val= str_replace( "::", "", trim( $val ) );
					$val= str_replace( "||", "", $val );
					$tmplvar_arr_prval[ $key ]= str_replace( "::", "", trim( $tmplvar_arr_prval[ $key ] ) );
					$tmplvar_arr_prval[ $key ]= str_replace( "||", "", $tmplvar_arr_prval[ $key ] );
					if( ! empty( $val ) ) $tmplvar .= ( ! empty( $tmplvar ) ? '||' : '' ) . $val .'::'. $tmplvar_arr_prval[ $key ];
				}
			}
		break;
		...
		//v005
			...
			{
				foreach( $tmplvar_arr_climg AS $key => $val )
				{
					$val= str_replace( "||", "", trim( $val ) );
