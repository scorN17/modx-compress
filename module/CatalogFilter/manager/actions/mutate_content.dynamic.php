...
<?php
	...
	if( $parentslist )
	{
		...
		{
			if( $row[ 'id' ] ) $qq .= ( ! empty( $qq ) ? " OR " : "" ) ."docs LIKE '%,{$row[id]},%'";
		}
	}
	if( $id > 0 && $qq )
	{
		$rs= $modx->db->select(
			"*",
			$modx->getFullTableName( '_cat_filter' ) ."",
			"( {$qq} ) AND type!=2 AND enabled=1 ORDER BY ii"
		);
	}elseif( $qq ){
		$rs= $modx->db->select(
			"*",
        ...
        while( $row= $modx->db->getRow( $rs ) )
		{
			if( true || $id > 0 )
			{
				...
				if( true || $docsqq )
				{
					$rr= $modx->db->select(
						...
						"idfilter={$row[id]} GROUP BY `value`"
					);
					if( $modx->db->getRecordCount( $rr ) )
					{
						while( $val= $modx->db->getRow( $rr ) )
						{
							if( ! empty( $val[ 'value' ] ) )
							{
								$props[ $row[ 'id' ] ][ $val[ 'id' ] ]= array( $val[ 'value' ], $val[ 'dopvalue' ] );
							}
						}
					}
				}
			...
			if( $limit2 > 0 )
			{
				while( $row2= $modx->db->getRow( $rs2 ) )
				{
					...
					print '<input type="text" id="doptv'. $row[ 'id' ] .'_'. $row2[ 'id' ] .'_tx2" name="doptv'. $row[ 'id' ] .'_'. $row2[ 'id' ] .'_tx2" value="'. $row2[ 'dopvalue' ] .'" tvtype="text" onchange="documentDirty=true;" style="width:200px;" />';
			...
			if( ( ! $limit2 && $row[ 'type' ] != 3 && $row[ 'type' ] != 5 ) || ( $row[ 'type' ] == 3 || $row[ 'type' ] == 5 ) )
			{
				for( $kk=1; $kk<=( $row[ 'type' ] == 3 || $row[ 'type' ] == 5 ? 5 : 1 ); $kk++ )
				{
					...
					print '<input type="text" id="doptv'. $row[ 'id' ] .'_x'. $kk .'_tx2" name="doptv'. $row[ 'id' ] .'_x'. $kk .'_tx2" value="'. $row2[ 'dopvalue' ] .'" tvtype="text" onchange="documentDirty=true;" style="width:200px;" />';
