...
if( $qq ) $rs= $modx->db->select(
	"cf.*",
	$modx->getFullTableName( '_cat_filter' ) ." AS cf",
	"( {$qq} ) AND cf.type!=2 AND cf.enabled=1 ORDER BY cf.id"
);
	...
	while( $row= $modx->db->getRow( $rs ) )
	{
		...
		for( $kk=1; $kk<=( $row[ 'type' ] == 3 ? 5 : 1 ); $kk++ )
		{
			...
			if( $row[ 'type' ] == 3 )
			{
				...
				$val= $modx->db->select( 'id', $modx->getFullTableName( '_cat_filter_value' ), "idfilter={$row[id]} AND iddoc={$key}", "", "1" );
				if( $modx->db->getRecordCount( $val ) == 1 )
				{
					$modx->db->update( array( 'value' => $doptmplvar, 'dopvalue' => $doptmplvar2 ), $modx->getFullTableName( '_cat_filter_value' ), "idfilter={$row[id]} AND iddoc={$key} LIMIT 1" );
				}else{
					$modx->db->insert( array( 'idfilter' => $row[ 'id' ], 'iddoc' => $key, 'value' => $doptmplvar, 'dopvalue' => $doptmplvar2 ), $modx->getFullTableName( '_cat_filter_value' ) );
				}
			}
...
if( $limit > 0 )
{
	while( $row= $modx->db->getRow( $rs ) )
	{
		$rs2= $modx->db->select(
			"id, `value`, dopvalue",
			$modx->getFullTableName( '_cat_filter_value' ) ."",
			"idfilter={$row[id]} AND iddoc='{$id}' ORDER BY id"
		);
		...
		if ($limit2 > 0)
		{
			while ($row2 = $modx->db->getRow($rs2))
			{
				...
				{
					$modx->db->delete( $modx->getFullTableName( '_cat_filter_value' ), "id={$row2[id]} AND idfilter={$row[id]} AND iddoc={$id} LIMIT 1" );
					continue 1;
				}
				$modx->db->update( array( 'value' => $doptmplvar, 'dopvalue' => $doptmplvar2 ), $modx->getFullTableName( '_cat_filter_value' ), "id={$row2[id]} AND idfilter={$row[id]} AND iddoc={$id} LIMIT 1" );
			}
		}
		...
		for( $kk=1; $kk<=( $row[ 'type' ] == 3 ? 5 : 1 ); $kk++ )
		{
			...
			if( $row[ 'type' ] == 3 )
			{
				...
				$val= $modx->db->select( 'id', $modx->getFullTableName( '_cat_filter_value' ), "idfilter={$row[id]} AND iddoc={$id}", "", "1" );
				if( $modx->db->getRecordCount( $val ) == 1 )
				{
					$modx->db->update( array( 'value' => $doptmplvar, 'dopvalue' => $doptmplvar2 ), $modx->getFullTableName( '_cat_filter_value' ), "idfilter={$row[id]} AND iddoc={$id} LIMIT 1" );
				}else{
					$modx->db->insert( array( 'idfilter' => $row[ 'id' ], 'iddoc' => $id, 'value' => $doptmplvar, 'dopvalue' => $doptmplvar2 ), $modx->getFullTableName( '_cat_filter_value' ) );
				}
			}
