	...
	if($rr && $modx->db->getRecordCount($rr))
	{
		...
		while($row= $modx->db->getRow($rr))
		{
			...
			if($row['file'] != $file)
			{
				if($file!==false)
				{
					...
					if($edit)
					{
						...
						if( ! file_exists($foo)) mkdir($foo, 0777, true);
			...
			if($flag)
			{
				...
				if($mypos!==false) $result= 'y';
				else{
					$pos= strpos($content, $row['search']);print $row['id'].'='.$pos.',';
					if($pos!==false)
					{
						$result= 'y';
						...
						if($row['type']=='before' || $row['type']=='replace') $newcontent .= $row['code'];
						if($row['type']!='replace') $newcontent .= $row['search'];
						if($row['type']=='after') $newcontent .= $row['code'];
