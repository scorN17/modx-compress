...
<?php
	...
	if( $parentslist )
	{
		...
		{
			if( $row[ 'id' ] ) $qq .= ( ! empty( $qq ) ? " OR " : "" ) ."cf.docs LIKE '%,{$row[id]},%'";
		}
	}
	if( $id > 0 && $qq )
	{
		$rs= $modx->db->select(
			...
			"( {$qq} ) AND cf.type!=2 AND cf.enabled=1"
		);
	}elseif( $qq ){
		$rs= $modx->db->select(
			"*",
	...
	if ($limit > 0)
	{
        ...
        while ($row = $modx->db->getRow($rs))
		{
			...
			print '<input type="text" id="doptv'. $row[ 'id' ] .'" name="doptv'. $row[ 'id' ] .'tx" value="'. $row[ 'value' ] .'" tvtype="text" onchange="documentDirty=true;" style="width:300px;">';
			
			print '<input type="text" id="doptv_'. $row[ 'id' ] .'" name="doptv'. $row[ 'id' ] .'tx2" value="'. $row[ 'dopvalue' ] .'" tvtype="text" onchange="documentDirty=true;" style="width:300px;">';
